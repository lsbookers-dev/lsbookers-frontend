...
export default function ArtistProfilePage() {
  ...
  const [descDraft, setDescDraft] = useState(artist.description)
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        setShowSoundcloud(
          p.showSoundcloud !== null && p.showSoundcloud !== undefined ? p.showSoundcloud : false
        )
  ...
  const handleSaveDescription = async () => {
    try {
      setDescSaving(true)
      const updated = await saveProfile({ bio: descDraft })

      const nextBio = updated?.bio ?? descDraft
      setDescription(nextBio || '')
      setDescDraft(nextBio || '')
      setEditingDesc(false)
      alert('Description enregistrée ✅')
    } catch (err) {
      console.error('Erreur de sauvegarde de la description:', err)
      alert("Impossible d'enregistrer la description.")
    } finally {
      setDescSaving(false)
    }
  }
  ...
  const toggleRole = async (label: string) => {
    ...
    try {
      ...
      setRoles(previous)
      alert("Impossible d'enregistrer les spécialités.")
    }
  }
  ...
  const onSelectBanner = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err)
      alert('Échec de sauvegarde de la bannière')
    } finally {
  ...
  const onSelectAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err)
      alert("Échec de sauvegarde de l'avatar")
    } finally {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-300">
              {location || artist.location}, {profile?.country ?? artist.country}
            </p>
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 pb-12">
        <div className="space-y-6">
                ...
                <>
                    ...
                    <div className="rounded-xl overflow-hidden border border-white/10 bg-black/30">
                      ...
                      <div className="p-3">
                        ...
                        {publications[0].caption && (
                          <p className="text-sm text-neutral-300 mt-1">{publications[0].caption}</p>
                        )}
                        {publications[0].time && (
                          <p className="text-xs text-neutral-400 mt-1">{publications[0].time}</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {publications.slice(1, 4).map((p) => (
                      <div
                        key={p.id}
                        className="rounded-xl overflow-hidden border border-white/10 bg-black/30 h-28"
                      >
              ...
              {!editingDesc ? (
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={descSaving}
                    className="text-sm px-3 py-1 rounded-full bg-pink-600 hover:bg-pink-500 disabled:opacity-60"
                  >
                    {descSaving ? 'Enregistrement…' : 'Enregistrer'}
                  </button>
          ...
          <section className="bg-neutral-900/60 border border-white/10 rounded-2xl p-4">
            ...
            <div
              id="calendar"
              className="mt-3 h-96 rounded-xl bg-black/30 border border-white/10"
            ></div>
          </section>
        </div>
