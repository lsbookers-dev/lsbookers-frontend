...
export default function OrganizerProfilePage() {
  ...
  // ——— socials / contacts (pas encore branchés backend V2)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setProfile(p)
        setOrgName(p.user?.name || userLite?.name || 'Mon Établissement')
  ...
  const toggleSpec = (s: string) => {
    setSpecialties((prev) => {
      ...
      if (profileId && token) {
        saveProfile({ specialties: next }).catch(() => {})
      }
  ...
  const saveSocials = async () => {
    alert('La sauvegarde des réseaux/contacts demandera la V2 backend. On le fera juste après.')
  }

  const saveDescription = async () => {
    try {
      setDescSaving(true)
      const updated = await saveProfile({ bio: description })
      if (updated?.bio !== undefined) {
        setDescription(updated.bio || '')
      }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* Mini panneau d’édition de zone */}
      {editingGeo && (
          ...
          <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div className="rounded-2xl overflow-hidden border border-white/10 bg-black/30">
              ...
              <div className="relative w-full h-72">
                <iframe title="map" src={mapSrc} className="absolute inset-0 w-full h-full" loading="lazy" />
              </div>
          ...
          <section className="bg-neutral-900/60 border border-white/10 rounded-2xl p-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={descSaving}
                className="text-sm px-3 py-1 rounded-full bg-white/10 hover:bg-white/20 disabled:opacity-60"
              >
          ...
          <section className="bg-neutral-900/60 border border-white/10 rounded-2xl p-4">
            ...
            <div className="mt-3 space-y-3">
              {[
                {
                  id: 1,
                  author: 'Soirée Riviera',
                  avatar: '/avatars/pro1.png',
                  rating: 5,
                  text: 'Top accueil & technique.',
                },
                {
                  id: 2,
                  author: 'Agence Eventa',
                  avatar: '/avatars/pro2.png',
                  rating: 4,
                  text: 'Equipe sérieuse, bon matos.',
                },
              ].map((r) => (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <span key={i} className={i < r.rating ? 'text-yellow-400' : 'text-neutral-600'}>
                            ★
                          </span>
                        ))}
                      </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewJob((j) => ({
                    ...j,
                    type: e.target.value as 'ARTIST' | 'PROVIDER' | 'ALL',
                  }))
                }
              >
