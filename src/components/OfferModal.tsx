...
import {
  X, MapPin, Calendar, User, Euro, Users,
  Send, ChevronRight, Search, CheckCircle, Briefcase,
} from 'lucide-react'
...
const TYPE_STYLE: Record<string, { border: string; dot: string; tag: string }> = {
  ARTIST:   { border: 'border-l-pink-500',   dot: 'bg-pink-400',   tag: 'bg-pink-500/15 text-pink-300 border-pink-500/20' },
  PROVIDER: { border: 'border-l-blue-500',   dot: 'bg-blue-400',   tag: 'bg-blue-500/15 text-blue-300 border-blue-500/20' },
  ALL:      { border: 'border-l-purple-500', dot: 'bg-purple-400', tag: 'bg-purple-500/15 text-purple-300 border-purple-500/20' },
}
...
export default function OfferModal({ offer, onClose, isLoggedIn }: Props) {
  ...
  return (
    ...
    >
      <div
        className={`relative max-w-lg w-full bg-neutral-950 border border-white/10 border-l-4 ${style.border} rounded-2xl overflow-hidden shadow-2xl`}
      ...
      >
        ...
        <div className="p-5 border-b border-white/10">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2.5">
              <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 mt-1.5 ${style.dot}`} />
              <div>
                <h2 className="font-bold text-white text-base leading-snug">{offer.title}</h2>
                {offer.specialty && (
                  <span className={`inline-block mt-1 text-[11px] px-2 py-0.5 rounded-full border font-medium ${style.tag}`}>
                    {offer.specialty}
                  </span>
                )}
              </div>
            ...
            <button
              onClick={onClose}
              className="bg-white/8 hover:bg-white/15 text-white rounded-full p-1.5 transition flex-shrink-0 mt-0.5"
            >
              <X size={15} />
            </button>
          </div>
        ...
        <div className="p-5 space-y-4 max-h-[55vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
            <div className="flex items-center gap-2 text-sm text-white/70">
              ...
              <span className="truncate font-medium">{offer.organizer.name}</span>
            </div>
          ...
          {applyStep === 'confirm' && (
            <div className="space-y-2.5 border border-white/10 rounded-xl p-3.5 bg-white/3">
              ...
              <div className="flex gap-2">
                ...
                >
                  {applying ? 'Envoi…' : 'Confirmer la candidature'}
                </button>
                <button
            ...
            <div className="border border-white/10 rounded-xl p-3.5 bg-white/3 space-y-3">
              {shareDone !== null ? (
                <div className="flex flex-col items-center gap-2 py-2 text-center">
                  ...
                  <button
                    onClick={() => router.push(`/messages?c=${shareDone}`)}
                ...
                <>
                  <p className="text-xs text-white/45 font-medium">Partager à un utilisateur</p>
                  ...
                  {shareLoading && (
                    <p className="text-xs text-white/30 text-center py-1">Recherche…</p>
                  )}
        ...
        <div className="border-t border-white/10 p-4 space-y-2">
          <div className="flex gap-2">
            ...
            {applyStep === 'idle' && (
              <button
                ...
                className="flex-1 bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold py-2.5 rounded-xl transition flex items-center justify-center gap-2"
              >
                <Briefcase size={14} /> Postuler
              </button>
            )}
            {applyStep === 'confirm' && (
              <div className="flex-1 bg-purple-600/20 border border-purple-500/30 text-purple-300 text-sm font-medium py-2.5 rounded-xl text-center">
                En cours…
              </div>
            )}
            {applyStep === 'done' && (
              <div className="flex-1 bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-medium py-2.5 rounded-xl text-center flex items-center justify-center gap-2">
            ...
            <button
              ...
              className={`px-3.5 py-2.5 rounded-xl text-sm transition border ${
                shareOpen
            ...
            <button
              onClick={goSimilar}
              title="Voir offres similaires"
              className="px-3.5 py-2.5 rounded-xl text-sm bg-white/5 border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition"
            >
          ...
          <div className="flex gap-2 text-[10px] text-white/20">
            <span className="flex-1 text-center">Postuler</span>
            <span className="w-12 text-center">Partager</span>
            <span className="w-12 text-center">Similaires</span>
          </div>
        </div>
      </div>
    </div>
